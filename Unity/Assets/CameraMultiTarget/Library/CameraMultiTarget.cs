...
public class CameraMultiTarget : MonoBehaviour
{
	...
	PositionAndRotation TargetPositionAndRotation(GameObject[] targets)
	{
		...
		var targetsRotatedToCameraIdentityList = new List<Vector3>();
		foreach (var target in targets)
		{
			if(target != null)
			targetsRotatedToCameraIdentityList.Add(inverseRotation * target.transform.position);
		}
		
		var targetsRotatedToCameraIdentity = targetsRotatedToCameraIdentityList.ToArray();
